describe('Basic user flow for Website', () => {
  ...
  for (let i = 0; i < prodItemsData.length; i++) {
    ...
    if (product.price === undefined || product.price === null || String(product.price).length === 0) {
      allArePopulated = false;
    }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    // Reset cart so this test starts clean
    await page.evaluate(() => {
    ...
    // Grab the button inside shadowRoot
    const button = await productItem.evaluateHandle(item => {
      return item.shadowRoot.querySelector('button');
    });
    // Click the button
    await button.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // Click the button inside each product-item's shadowRoot
    for (const item of productItems) {
      const button = await item.evaluateHandle(product => {
      return product.shadowRoot.querySelector('button');
    });
    await button.click();
}

// Check that the cart count says 20
...
expect(cartCount).toBe('20');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     */
    await page.reload();
    // Get the button text from every product-item
    const buttonTexts = await page.$$eval('product-item', items => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (const item of productItems) {
      ...
      expect(await button.evaluate(btn => btn.innerText)).toBe('Add to Cart');
    }
